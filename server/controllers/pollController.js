...
const postPoll = async (req, res) => {
  ...
  if (!ownerId || !title || !options) return res.status(404); // replace with 404 Error
  ...
  await poll.save();
  return res.status(200).json(poll); // replace with status code 200 response
};
...
const postVote = async (req, res) => {
  ...
  if (!pollId || !optionId) return res.status(400); // update for 400 Error
  ...
  if (updateOption.modifiedCount == 0) return res.status(400).send("Error: failed to update poll"); // update for 400 Error

  const updatedPoll = await Poll.findById(pollId);

  console.log(`Vote cast for ${pollId} on option ${optionId}`);

  return res.status(200).json(updatedPoll); // update for status code 200 res
};
